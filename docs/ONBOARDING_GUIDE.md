...
این پروژه ساختار اداره (Governance) سختگیرانه‌ای دارد. **قبل از هر تغییر در کد**، باید **`docs/constitution/main.md`** را خوانده باشید. نه فقط برای style — برای جلوگیری از خراب کردن کار قبلی.
...
### قدم ۳.۱ — Constitution (۳۰ دقیقه) ⭐ مهم‌ترین

```
docs/constitution/main.md
```

این سند **مهم‌ترین** سند است. Constitution مدولار v2.18 شامل:
- اصول بنیادی پروژه (۹۰ قانون قفل‌شده)
- معماری ۱۲-سندی (مدولار)
- پروتکل شروع/پایان چت
...
نگاه کنید به `claude_workspace/CHAT_BOOT_TRIGGER_TEMPLATE.md` برای پروتکل شروع چت + `docs/constitution/main.md` بخش سسیون و متا.

### Q9: اگر در میانه کار Claude اشتباه کرد؟

طبق قوانین Trust (#۷۸-#۸۵) در Constitution، Claude باید به نقد سازنده پاسخ دهد. به خطا صریحاً اشاره کنید.
