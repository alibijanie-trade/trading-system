# -*- coding: utf-8 -*-
"""
07_end_of_chat3_update_docs.py  (نسخه نهایی برای v2.3)
═══════════════════════════════════════════════════════════
آپدیت پایان چت ۳ — تثبیت اسناد + سند ۱۴ پروتکل تعویض چت
+ پروتکل پایان طبیعی چت (v2.3)
...
⚠️ این نسخه جایگزین نسخه قبلی همین فایل است.
   اگر نسخه قبلی را اجرا کرده‌اید، این را دوباره اجرا کنید
   تا اسناد به v2.3 آپدیت شوند.
═══════════════════════════════════════════════════════════
"""
...
print("=" * 64)
print("📝 آپدیت اسناد Session — پایان چت ۳ (نسخه v2.3)")
print("=" * 64)
...
# ═══════════════════════════════════════════════════════════
SESSION_STATUS = """# وضعیت پروژه — 2026-05-15 (پایان چت ۳: تثبیت اسناد + v2.3)
...
| **۱۵ 🆕 v2.2/v2.3** | اجرای اجباری پروتکل تعویض چت (سند ۱۴) — هم در افت کیفیت، هم در پایان طبیعی چت |
...
## رفتار اجباری Claude در پایان چت (🆕 v2.3)

طبق بند ۱۴.۴ سند جامع v2.3:

اگر کاربر یکی از این عبارات را گفت → Claude باید **قبل از پاسخ خداحافظی** بپرسد آیا اسکریپت آپدیت اسناد session ساخته شود:

- «تمام می‌کنم» / «تمام شد» / «تمومش می‌کنم»
- «خداحافظ» / «بای»
- «برای امروز کافی است» / «امروز بسه»
- «چت را می‌بندم» / «این چت تمام»
- «استراحت می‌کنم» / «بعداً ادامه می‌دهم»
- «فردا ادامه می‌دهیم»
- "bye" / "done for today" / "see you later"

پرسش پیشنهادی: «آیا می‌خواهید قبل از بستن چت، اسکریپت آپدیت SESSION_STATUS و PROJECT_CONTEXT را بسازم؟»
...
### چت ۳ — تثبیت اسناد (۸ تصمیم)

18. **افزودن سند ۱۴ — پروتکل تعویض چت** (Chat Handoff Protocol)
19. **قانون قفل‌شده #15** — اجرای اجباری پروتکل سند ۱۴
20. **انتقال سند ۱۴ قدیمی (وضعیت اجرا) به سند ۱۵**
21. **افزایش نسخه سند جامع به v2.2**
22. **فرمت پیش‌فرض سند جامع: Markdown**
23. **🆕 افزودن بخش ۱۴.۴ — پروتکل پایان طبیعی چت** (v2.3)
24. **🆕 بازتعریف قانون #15** — حالا هم افت کیفیت و هم پایان طبیعی چت را پوشش می‌دهد (v2.3)
25. **🆕 افزایش نسخه سند جامع به v2.3** — نسخه نهایی این چت

## سند مرجع

- **سند جامع فعلی:** v2.3 (در فایل `سند_جامع_v2.3.md`)
...
print()
print("=" * 64)
print("✅ اسناد Session آپدیت شدند (نسخه نهایی v2.3).")
print("   اکنون می‌توانید چت را ببندید.")
print("=" * 64)
print()
print("📌 برای چت جدید، ۴ مورد را پیوست کنید:")
print("   ۱. سند_جامع_v2.3.md (از پنل سمت راست چت قبلی — نسخه نهایی)")
