# -*- coding: utf-8 -*-
"""
51_create_api_docs.py — تولید docs/API_DOCS.md (T2.09)

مستند رسمی API پروژه — ۶ endpoint فاز ۰:
  - GET  /api/v1/health
  - POST /api/v1/auth/login   (OAuth2 form)
  - POST /api/v1/auth/refresh
  - POST /api/v1/auth/logout  (auth)
  - GET  /api/v1/auth/me      (auth)
  - GET  /api/v1/ohlcv/{symbol_id} (auth + pagination)

استفاده (از ریشه پروژه):
  python scripts\\51_create_api_docs.py
"""
...
API_DOCS_CONTENT = """# API Documentation — سامانه هوشمند ترید
...
def main() -> int:
    print("=" * 64)
    print("  51_create_api_docs — تولید docs/API_DOCS.md")
    print("=" * 64)
    print()

    written = write_if_changed(DOC, API_DOCS_CONTENT)
    if written:
        print(f"  ✏️  ایجاد/به‌روزرسانی: {DOC.name}")
        print(f"  📏  حجم: {len(API_DOCS_CONTENT):,} کاراکتر")
        print(f"  📄  تعداد خط: {API_DOCS_CONTENT.count(chr(10))}")
    else:
