...
- **`<part20-hash>`** = chat-end part20 (governance repair Batch 1+2: header v2.14→v2.17 در ۶ ماژول + LOCKED_RULES_INBOX در Quick-start + PHASE_LEDGER در STEP 1 boot + SESSION_STATUS rewrite از part13→part19 + split 01_rules.md→01a_rules_core + shell standard CMD→PowerShell + PHASE1_PART21_HANDOFF) — backfill هش در boot part21 (M101، از git زنده).
