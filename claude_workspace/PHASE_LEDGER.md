...
- **`<chat-end part19>`** = part19 chat-end (ردیف Ledger part19 + PART20 handoff + backfill #۰۱۲=`dcb5c20`) — backfill هش در boot part20 (M101). زنجیرهٔ commitهای part19: `dcb5c20` (check_13/Review #۰۱۲) → `104abf0` (CHAT_LOG catch-up) → `a6300ce` (Quick-Lock + بازیابی کادر) → chat-end (این commit).
